...
$websitefee = floatval(mysqli_real_escape_string($conn,$_POST['websitefee']));
...
$digitalfee = floatval(mysqli_real_escape_string($conn,$_POST['digitalfee']));
$appfee = floatval(mysqli_real_escape_string($conn,$_POST['appfee']));
$financialfee = floatval(mysqli_real_escape_string($conn,$_POST['financialfee']));
$applicationfee = floatval(mysqli_real_escape_string($conn,$_POST['applicationfee']));
$certificatefee = floatval(mysqli_real_escape_string($conn,$_POST['certificatefee']));
$others = floatval(mysqli_real_escape_string($conn,$_POST['others']));

if($paid>0&&$totalamount>0){
$sql = "select contact,balance  from client where id = '$stdid'";
$sq = $conn->query($sql);
$sr = $sq->fetch_assoc();
$contact = $sr['contact'];
$tbalance=$sr['balance'];
$sql = "select totalamount,paid  from onlinepay_transaction where id = '$sid'";
$sq = $conn->query($sql);
$sr = $sq->fetch_assoc();
$oldbalance = $sr['totalamount']-$sr['paid'];
$sql = "UPDATE onlinepay_transaction SET submitdate='$submitdate',websitefee='$websitefee',digitalfee='$digitalfee',appfee='$appfee',financialfee='$financialfee',applicationfee='$applicationfee',certificatefee='$certificatefee',others='$others',transcation_remark='$transaction_remark',paid='$paid',total_deduction='$tdeduction',totalamount='$totalamount' WHERE id='$sid'";
$conn->query($sql);
$tbalance =$tbalance-$oldbalance+($totalamount- $paid) ;
$sql = "update client set balance='$tbalance' where id = '$stdid'";
$conn->query($sql);
 echo '<script type="text/javascript">window.location="onlineservicereport.php?act=1";</script>';}
 else{
  echo '<script type="text/javascript">window.location="onlineservicereport.php?act=2";</script>';
}
}
if(isset($_REQUEST['act']) && @$_REQUEST['act']=="1")
{
$errormsg = "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Success!</strong>Online Service pay Edit Successfully</div>";
}
...
{
$errormsg = "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>failure!</strong>you already added a payment for selected month for the client</div>";
}
if(isset($_GET['action']) && $_GET['action']=="delete"){


  $sqll = "select * from onlinepay_transaction where id='".$_GET['id']."'";
  $sqll = $conn->query($sqll);
  $srl = $sqll->fetch_assoc();	
  $spaid=$srl['paid'];
  $sbtpay=$srl['totalamount'];
  $stdid=$srl['stdid'];
  $sql = "select balance  from client where id ='".$stdid."'";
  $sq = $conn->query($sql);
  $sr = $sq->fetch_assoc();	
  $sb=$sr['balance'];
  $balance =$sb-($sbtpay-$spaid);
  $conn->query("UPDATE client SET balance='$balance' WHERE id='".$stdid."'");
  $conn->query("DELETE FROM onlinepay_transaction WHERE id='".$_GET['id']."'");
  
    header("location: onlineservicereport.php");
...
?>
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">Online Service Payment Report  
...
<form class="form-inline" role="form" id="searchform" action="functions.php" method="post" name="upload_excel"   
  ...
  <div class="form-group">
    ...
    <select  class="form-control" id="branch" name="branch" >
		<option value="" >Select Department</option>
                                    <?php
									$sql = "select * from branch where delete_status='0' and id='3' order by branch.branch  asc";
									$q = $conn->query($sql);
  ...
  <button type="submit" class="btn btn-danger btn-sm" id="OnlineExport" name="OnlineExport" > Export </button>
              ...
              $("#subjectresult").html('<table class="table table-striped table-bordered table-hover" id="tSortable23"><thead><tr><th>Name/Contact</th><th>Pay Amount</th><th>Paid</th><th>Balance</th><th>Payment Date</th><th>View</th><th>Edit</th></tr></thead><tbody></tbody></table>');
			    ...
			    $("#tSortable23").dataTable({
                  ...
                  "bInfo": false,
							       'bProcessing' : true,
							       'bServerSide': true,
							       'sAjaxSource': "datatable.php?"+$('#searchform').serialize()+"&type=onlineservicereport",
							       'aoColumnDefs': [{
                                   'bSortable': false,
...
////////////////////////////
 $("#tSortable23").dataTable({
                  ...
                  'bProcessing' : true,
				  'bServerSide': true,
                  'sAjaxSource': "datatable.php?type=onlineservicereport",
