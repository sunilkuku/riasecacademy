...
$contact = $sr['contact'];
$tbalance=$sr['balance'];
$sql = "select  balacetopay,paid from fees_transaction where id = '$sid'";
$sq = $conn->query($sql);
$sr = $sq->fetch_assoc();
$oldbalance=$sr['balacetopay']-$sr['paid'];
$sql = "UPDATE fees_transaction SET submitdate='$submitdate',counsellingfee='$counsellingfee',monitoringfee='$monitoringfee',aptitudefee='$aptitudefee',personalityfee='$personalityfee',assessmentfee='$assessmentfee',transcation_remark='$transaction_remark',paid='$paid',totalfee='$totalfee',total_deduction='$tdeduction',balacetopay='$btpay',othexpense='$othex' WHERE id='$sid'";
$conn->query($sql);
$tbalance =$tbalance-$oldbalance+($btpay- $paid) ;
$sql = "update student set balance='$tbalance' where id = '$stdid'";
$conn->query($sql);
 echo '<script type="text/javascript">window.location="careerreport.php?act=1";</script>';}
 else{
  echo '<script type="text/javascript">window.location="careerreport.php?act=2";</script>';
}
}
if(isset($_REQUEST['act']) && @$_REQUEST['act']=="1")
{
$errormsg = "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Success!</strong>Career Guidance Payment Edit Successfully</div>";
}
...
if(isset($_GET['action']) && $_GET['action']=="delete"){
  ...
  $sqll = "select * from fees_transaction where id='".$_GET['id']."'";
  ...
  $conn->query("DELETE FROM fees_transaction WHERE id='".$_GET['id']."'");
  
    header("location: careerreport.php");
...
?>
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">Career Guidance Report  
...
<form class="form-inline" role="form" id="searchform" action="functions.php" method="post" name="upload_excel"   
  ...
  <button type="submit" class="btn btn-danger btn-sm" id="CareerExport" name="CareerExport" > Export </button>
...
function mydatatable()
{
              ...
              $("#subjectresult").html('<table class="table table-striped table-bordered table-hover" id="tSortable22"><thead><tr><th>Name/Contact</th><th>Pay Amount</th><th>Paid</th><th>Balance</th><th>Payment Date</th><th>View</th><th>Edit</th></tr></thead><tbody></tbody></table>');
			    ...
			    $("#tSortable22").dataTable({
                  ...
                  "bInfo": false,
							       'bProcessing' : true,
							       'bServerSide': true,
							       'sAjaxSource': "datatable.php?"+$('#searchform').serialize()+"&type=monthlyreport",
							       'aoColumnDefs': [{
                                   'bSortable': false,
...
////////////////////////////
 $("#tSortable22").dataTable({
                  ...
                  'bProcessing' : true,
				  'bServerSide': true,
                  'sAjaxSource': "datatable.php?type=monthlyreport",
...
$.ajax({
            type: 'post',
            url: 'getfeeform.php',
            data: {student:sid,dat:sdt,req:'3'},
            success: function (data) {
