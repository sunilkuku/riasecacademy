...
$admissionfee = floatval(mysqli_real_escape_string($conn,$_POST['admissionfee']));
...
$secondaryfee = floatval(mysqli_real_escape_string($conn,$_POST['secondaryfee']));
$seniorsecondary = floatval(mysqli_real_escape_string($conn,$_POST['seniorsecondary']));
$ugfee = floatval(mysqli_real_escape_string($conn,$_POST['ugfee']));
$pgfee = floatval(mysqli_real_escape_string($conn,$_POST['pgfee']));
$diplomafee = floatval(mysqli_real_escape_string($conn,$_POST['diplomafee']));
$regfee = floatval(mysqli_real_escape_string($conn,$_POST['regfee']));
$examfee = floatval(mysqli_real_escape_string($conn,$_POST['examfee']));
$materialsfee = floatval(mysqli_real_escape_string($conn,$_POST['materialsfee']));
...
$contact = $sr['contact'];
$tbalance=$sr['balance'];
$sql = "select totalamount,paid  from virtual_transaction where id = '$sid'";
$sq = $conn->query($sql);
$sr = $sq->fetch_assoc();
$oldbalance = $sr['totalamount']-$sr['paid'];
$sql = "UPDATE virtual_transaction SET submitdate='$submitdate',admissionfee='$admissionfee',secondaryfee='$secondaryfee',seniorsecondary='$seniorsecondary',ugfee='$ugfee',pgfee='$pgfee',diplomafee='$diplomafee',regfee='$regfee',examfee='$examfee',materialsfee='$materialsfee',miscellaneousfee='$miscellaneousfee',transcation_remark='$transaction_remark',paid='$paid',total_deduction='$tdeduction',totalamount='$totalamount' WHERE id='$sid'";
$conn->query($sql);
$tbalance =$tbalance-$oldbalance+($totalamount- $paid) ;
$sql = "update student set balance='$tbalance' where id = '$stdid'";
$conn->query($sql);
 echo '<script type="text/javascript">window.location="virtualreport.php?act=1";</script>';}
 else{
  echo '<script type="text/javascript">window.location="virtualreport.php?act=2";</script>';
}
}
if(isset($_REQUEST['act']) && @$_REQUEST['act']=="1")
{
$errormsg = "<div class='alert alert-success'><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a><strong>Success!</strong>Virtual Academy Fee Edit Successfully</div>";
}
...
if(isset($_GET['action']) && $_GET['action']=="delete"){
  ...
  $sqll = "select * from virtual_transaction where id='".$_GET['id']."'";
  ...
  $conn->query("DELETE FROM virtual_transaction WHERE id='".$_GET['id']."'");
  
    header("location: virtualreport.php");
...
?>
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h1 class="page-head-line">Virtual Academy Payment Report  
...
<form class="form-inline" role="form" id="searchform" action="functions.php" method="post" name="upload_excel"   
  ...
  <div class="form-group">
    ...
    <select  class="form-control" id="branch" name="branch" >
		<option value="" >Select Department</option>
                                    <?php
									$sql = "select * from branch where delete_status='0' and id='1' order by branch.branch  asc";
									$q = $conn->query($sql);
  ...
  <button type="submit" class="btn btn-danger btn-sm" id="VirtualExport" name="VirtualExport" > Export </button>
              ...
              $("#subjectresult").html('<table class="table table-striped table-bordered table-hover" id="tSortable23"><thead><tr><th>Name/Contact</th><th>Pay Amount</th><th>Paid</th><th>Balance</th><th>Payment Date</th><th>View</th><th>Edit</th></tr></thead><tbody></tbody></table>');
			    ...
			    $("#tSortable23").dataTable({
                  ...
                  "bInfo": false,
							       'bProcessing' : true,
							       'bServerSide': true,
							       'sAjaxSource': "datatable.php?"+$('#searchform').serialize()+"&type=virtualreport",
							       'aoColumnDefs': [{
                                   'bSortable': false,
...
////////////////////////////
 $("#tSortable23").dataTable({
                  ...
                  'bProcessing' : true,
				  'bServerSide': true,
                  'sAjaxSource': "datatable.php?type=virtualreport",
...
$.ajax({
            type: 'post',
            url: 'getfeeform.php',
            data: {student:sid,dat:sdt,req:'9'},
            success: function (data) {
